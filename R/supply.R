...
#' @param xmax Numeric. Allows modifying the maximum X value for the default supply function.
#' @param ymax Numeric. Allows modifying the maximum Y value for the default supply function.
#' @param type Possible values are `"convex"` (default) and `"line"` to plot a convex or a linear supply function by default, respectively.
...
supply <- function(...,
  ...
  if(missing(...)){
    ...
    if(type == "convex") {
      ...
      curve <- data.frame(Hmisc::bezier(c(1, 8, xmax),
                             c(1, 5, ymax)))
  ...
  if(curve_names == TRUE) {
    ...
    if(ncurves == 1) {
      ...
      p <- p + annotate(geom = "text", x = max(as.data.frame(curve)$x) + 0.5, y = max(as.data.frame(curve)$y), label = names, parse = TRUE,
      ...
      j <- 0
      for(i in 1:ncurves){
        p <- p + annotate(geom = "text", x = max(as.data.frame(curve)$x) + j + 0.35, y = max(as.data.frame(curve)$y), label = names[i], parse = TRUE,
                          size = 4, color = geomcol)
        j <- j + 1
      }
    }
